...
multitude of seams, and Audacious's scrims are light and less concealing. Push the
...
```css
.ad-loop-a, .ad-loop-b { position: absolute; inset: 0; object-fit: cover; }
.ad-loop-a { animation: ad-loop-fade 12s linear infinite; }
.ad-loop-b { animation: ad-loop-fade 12s linear infinite -6s; }
@keyframes ad-loop-fade { 0%, 40% { opacity: 1 } 50%, 90% { opacity: 0 } 100% { opacity: 1 } }
```
...
- [ ] **Re-run the contrast audit.** A swapped asset changes what's behind live
      ...
      expected still can push `--ad-ink` over a scrim below 4.5:1, and it will
