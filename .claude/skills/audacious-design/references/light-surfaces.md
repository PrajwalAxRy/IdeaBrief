...
`opacity: 0` and get skipped. Run it again inside any `.ad-invert` band — the
...
|---|---|---|---|---|
| `--ad-ink` | 17.85 | 16.96 | 15.83 | 14.36 |
| `--ad-body` | 7.83 | 7.44 | 6.95 | 6.30 |
| `--ad-muted` | 5.00 | 4.75 | **4.44** | **4.02** |
| `--ad-faint` | **3.05** | **2.90** | **2.71** | **2.46** |
| `--ad-accent` | **3.39** | **3.22** | **3.01** | **2.73** |
| `--ad-accent-text` | 6.04 | 5.74 | 5.36 | 4.86 |
| `--ad-positive` | 6.51 | 6.19 | 5.78 | 5.24 |
| `--ad-caution` | 6.78 | 6.45 | 6.02 | 5.46 |
| `--ad-critical` | 6.35 | 6.03 | 5.63 | 5.11 |
| `--ad-info` | 7.47 | 7.10 | 6.62 | 6.01 |
...
**The rule:** body copy is `--ad-body` (`#57514A`, 6.30:1 at worst). `--ad-muted`
is for *secondary* copy and metadata, and only on `--ad-paper` and `--ad-canvas`.
On `--ad-linen` and `--ad-inset` it drops to 4.44 and 4.02 — use `--ad-body`.

`--ad-faint` is never text. Not for timestamps, not for placeholder copy, not
...
**The rule:** `--ad-accent` is for *marks* — a dot, a rule, a bar, a fill, a
chart series. Every accent-coloured **character** uses `--ad-accent-text`
(`#AE3D12`, 4.86:1 at worst).

Colour is also not an affordance on its own. An inline prose link needs an
underline (`.ad-link-inline` has one); a link that's just coloured text is
...
(`--ad-accent-solid`, 5.16:1), which is muddy at button scale and starts
competing with `--ad-action` for the same meaning. That's a dead end.

**The rule:** the button is `--ad-action`. The accent sits beside it as a text
...
`--ad-ink` on `--ad-accent` (5.27:1), not white.

### 2.4 The invisible hairline

`--ad-line` against each ground:
...
panel. `.ad-well` deliberately ships with *no* border for exactly this reason —
...
**The rule:** shadows are warm. `--ad-lift` and `--ad-lift-panel` use
`rgba(61,48,36,…)`. Inside `.ad-invert` they switch back toward neutral black,
...
This is the real reason `--ad-canvas` is the *default* ground and `--ad-paper`
is reserved for card faces and one of two alternating bands. Pure white is a
material in this system, not the substrate.

Practical consequences:

- **Never stack two identical bands.** Two `--ad-band-paper` sections in a row
  produce a 256px void where a boundary should be. Alternate.
- **A long-form section takes `--ad-canvas` or `--ad-linen`,** never paper. Text
...
`.ad-invert` remaps the same token names to dark values. Components don't know
...
**A nested `.ad-invert`.** The inner one re-applies already-inverted values, so
nothing changes and the intent silently fails. Inversion is not a toggle.

**`--ad-faint` still fails.** It's 2.50:1 at worst on dark grounds — same
category as light, same rule: never text.

Inside an inverted band, `--ad-accent-text` remaps to `#FF7A4D` (5.79:1 at
...
| The element… | Gets |
|---|---|
| sits flat in a grid | `--ad-line`, no shadow |
| is a panel inside a card | one tier step, `--ad-line` |
| is the deepest well | one tier step, **no border** |
| floats over a photo or another card | `--ad-lift` |
| is the active thumb of a control | `--ad-lift` |
| floats over the page — modal, popover | `--ad-lift-panel` |
| is a hero product surface | `--ad-lift-panel` |
| is chosen or selected | `--ad-line-ink`, no shadow |
...
- **Scrims go light, not dark.** To place text over an image here you lighten
  the image (a white or `--ad-paper` scrim at 60–80%) and use `--ad-ink`. A
...
- **Give every image a hairline and a radius.** A full-bleed photo with a hard
  edge against warm paper looks unfinished; `--ad-line` at `--ad-r-lg` makes it
