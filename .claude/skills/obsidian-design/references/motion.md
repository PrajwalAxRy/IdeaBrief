...
```css
--ob-ease:  cubic-bezier(0.16, 1, 0.3, 1);   /* the only easing in the system */
--ob-fast:  180ms;   /* press, tap-back, icon nudge */
--ob-base:  320ms;   /* hover, colour, border, condense */
--ob-enter: 900ms;   /* reveal, mask, the verification rule drawing */
```
...
.ob-reveal {
  ...
  filter: blur(8px);
  transition:
    opacity var(--ob-enter) var(--ob-ease),
    transform var(--ob-enter) var(--ob-ease),
    filter var(--ob-enter) var(--ob-ease);
  transition-delay: var(--ob-reveal-delay, 0ms);
}
.ob-reveal[data-shown='true'] { opacity: 1; transform: none; filter: blur(0); }
```
...
export function ScrollReveal({ children, delay = 0, className, threshold = 0.15 }) {
  ...
  return (
    <div
      ...
      data-shown={inView}
      style={{ '--ob-reveal-delay': `${delay}ms` }}
    >
      {children}
    </div>
  );
...
```css
.ob-word-line { display: block; overflow: hidden; padding-bottom: 0.08em; }
.ob-word {
  display: inline-block;
  transform: translateY(105%);
  transition: transform var(--ob-enter) var(--ob-ease);
  transition-delay: var(--ob-word-delay, 0ms);
  white-space: pre;
}
[data-shown='true'] .ob-word { transform: none; }
```
...
lines.map(line => (
  <span className="ob-word-line" key={line}>
    {line.split(' ').map((word, i, all) => {
      wordIndex += 1;
      return (
        <span key={`${word}-${i}`} className="ob-word"
              style={{ '--ob-word-delay': `${delay + wordIndex * stagger}ms` }}>
...
- Make the rail's ticks real `<button>`s that scroll to their panel with
  `block: 'center'`.
- Dim inactive panels to ~0.42 opacity. Lower and it reads as disabled.
- **Check `overflow` on ancestors** — see `pitfalls.md` §5.

### 4c. Marquee

```css
.ob-marquee { overflow: hidden; mask-image: linear-gradient(90deg, transparent, #000 12%, #000 88%, transparent); }
.ob-marquee-track { display: flex; width: max-content; animation: ob-marquee 46s linear infinite; }
.ob-marquee:hover .ob-marquee-track { animation-play-state: paused; }
...
```css
.ob-verify-rule {
  height: 1px; background: var(--ob-accent);
  transform: scaleX(0); transform-origin: left center;
  transition: transform var(--ob-enter) var(--ob-ease);
}
[data-state='verified'] .ob-verify-rule,
[data-state='discarded'] .ob-verify-rule { transform: scaleX(1); }
[data-state='discarded'] .ob-verify-rule { background: var(--ob-discard); }
```
...
function TypingBubble({ turn, onDone }) {
  ...
  return <Bubble turn={turn}>{turn.text.slice(0, count)}<span className="ob-caret" /></Bubble>;
}
```
...
- Grain overlay (one global fixed pseudo-element with an SVG turbulence
  data-URI at ~0.035 opacity).
