...
`@layer utilities`. If `riley.css` is imported unlayered:

```css
/* unlayered — wins */        /* @layer utilities — loses */
.rl-h1  { margin: 0; }        .mt-8  { margin-top: 2rem; }
.rl-btn { gap: 8px; }         .gap-2 { gap: 0.5rem; }
```
...
@import "tailwindcss";
@import "./tokens.css";
@import "./riley.css" layer(components);   /* ← the layer() is the fix */
```
...
```js
() => {
  const el = document.querySelector('#some-headline');   // has "rl-h1 mt-8"
...
```css
.thing {
  box-shadow: 0 0 0 2px var(--rl-paper), 0 0 0 4px var(--rl-acccent-ring);
  /*                                                  ↑ typo              */
}
...
```bash
grep -oE 'var\(--[a-z0-9-]+' styles/riley.css | sort -u   # referenced
...
() => {
  const el = document.querySelector('.rl-tooltip');
  const out = [];
...
Killing animations without resolving their end state. `.rl-reveal` starts at
...
`recipes.css` §17 does this for `.rl-reveal`; anything you add with an
...
Scope the selector past the decoy (`main .rl-btn--primary`), or
...
```css
.rl-page { position: relative; z-index: 1; }   /* on the CONTENT container */
```
...
## 11. `--rl-body` is a colour and `--rl-body-size` is a size

### Symptom

`font-size: var(--rl-body)` renders at the browser default, or a size
declaration silently does nothing.

### Cause

A genuine naming collision in the token file. `--rl-body` is the **colour**
token for running prose; the **size** is `--rl-body-size`. Every other text
token (`--rl-sm`, `--rl-xs`, `--rl-sub`) is a size with no colour counterpart,
so the instinct to type `var(--rl-body)` for 16px is strong and wrong.
...
Use the `.rl-body` recipe class rather than the raw token. It sets both
correctly. If you must write it by hand, `--rl-body-size` for the size.

---

## 12. A nested `.rl-invert` does nothing
...
`.rl-invert` remaps token names to dark values. Applied inside an already
inverted subtree, it re-declares the same dark values — there's no toggle
semantics, so nothing flips back.

### Fix

Inversion is punctuation, not a switch. One band, one `.rl-invert`, at the top
...
`--rl-line` against `--rl-inset` is **1.05:1** — it does not render. Against
`--rl-linen` it's 1.15:1, which is visible but weak enough that a
linen-on-linen card reads as unbounded.

### Fix

Rule 3, the one-step rule. White card on linen, linen panel inside a white card,
inset well inside a linen panel. `.rl-well` ships with no border deliberately —
...
```js
() => {
  const el = document.querySelector('.rl-card');
