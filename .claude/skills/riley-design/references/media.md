...
This file matters more here than in a text-led system, because Riley's text
...
chart.** Read the `dataviz` skill first; Riley supplies the ground, hairline and
...
compliance, uptime? → **One 64px line icon** at 3px stroke in `--rl-accent`,
centred in an empty area (`.rl-card-icon`). Reach for this only after 1–3 have
...
for. The single exception is `.rl-bloom` — a static warm glow **tethered to the
...
**And if none of the six fits, ask for a reference** (SKILL.md rule 11) before
...
So: build the fragment. It goes in `.rl-bento__visual`, on `--rl-linen`, and it
uses the same chips, rows and type scale as the real product.

```
┌───────────────────────────────────────────────┐
│ ░░ .rl-bento__visual — on --rl-linen ░░░░░░░░ │
│                                               │
│   ┌─ .rl-row ────────────────────────────┐   │
...
│   └──────────────────────────────────────┘   │
│   ┌─ .rl-row ────────────────────────────┐   │
│   │ ~~◆ Vendor blog~~   [ DISCARDED ]    │   │  ← .rl-discarded, goes faint
│   └──────────────────────────────────────┘   │
├───────────────────────────────────────────────┤
│  Every source is checked                      │  ← .rl-h3
...
- An evidence list where one row is struck through and marked `DISCARDED`
  ...
  rejected thing goes `--rl-faint` and stops mattering.
...
owns form selection, palette validation, mark specs and interaction. Riley only
supplies the frame.

What Riley contributes:

- Ground: the chart sits on `--rl-paper` or inside a `.rl-panel`.
- Gridlines: `--rl-line`. Axis labels: `.rl-meta`, so they join the metadata
  layer rather than inventing a fourth type treatment.
- Series: `--rl-series-1` … `--rl-series-6`. Series 1 is the accent on purpose —
...
Two Riley-specific cautions:

- **Don't put a `--rl-critical` mark inside an accent-marked chart.** The two
  hues are close enough to collide and the status stops reading as a status.
- **Every figure needs a source.** `.rl-stat__source` exists for this. A number
...
| Shape | For | Recipe |
|---|---|---|
| **Connector-linked steps** | A sequence. 3–5 items. | `.rl-steps` — the connector is a hairline on the *container*, behind the cards. A per-card connector overhangs the last one. |
| **Ring of orbiting tiles** | Many things connecting to one thing — integrations, sources. | Concentric `--rl-line` circles, tiles positioned by `cos/sin` at a fixed radius, centre logo on `--rl-paper` with `--rl-lift`. |
| **Two-column mapping** | A before/after, or an input/output. | Two `.rl-panel`s with an arrow between. Resist labelling both columns — the shape says it. |
...
| Constraint | Why |
|---|---|
| **Scrims go light, not dark.** A white or `--rl-paper` veil at 60–80%, with `--rl-ink` text over it. | A dark scrim plus white text is a dark-theme move and reads as a foreign object dropped into the page. |
...
| **Every image gets a hairline and a radius.** `--rl-line` at `--rl-r-lg`. | A full-bleed photo with a hard edge against warm paper looks unfinished. The opposite of the dark-system instinct, where a border reads as a frame nobody asked for. |
...
```css
.rl-photo { position: relative; overflow: hidden; border-radius: var(--rl-r-lg); }
.rl-photo img { filter: grayscale(0.35) contrast(0.96) brightness(1.06); }
.rl-photo::after {
  content: ''; position: absolute; inset: 0;
  background:
    linear-gradient(to right, var(--rl-paper) 0%, rgba(255,255,255,0.55) 46%, transparent 72%);
}
```
...
- **Reserve the exact final height.** Filling a slot must cause zero layout
  shift.
- **A slot must look deliberate, not broken.** On `--rl-linen`, with a dashed
  `--rl-line-strong` border and `.rl-meta` label. A visitor should read it as
