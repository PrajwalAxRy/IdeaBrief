 ...
 * `lib/content/trial4.ts`; this hook only controls how it arrives. It stands
 * where the real Define seam would put the Vercel AI SDK's `useChat` — see
 * CLAUDE.md §"The three seams" — so a future swap replaces the source of `full`
 * and leaves every caller alone.
 *
 ...
 * cannot be reached by the CSS blanket in `obsidian.css`, and stopping the
 * timer without settling would leave the reply frozen mid-sentence — a partial
 * state, which is worse than the animation it was meant to spare.
 *
