/**
 * Content for `/trial4` — the Obsidian workspace concept.
 *
 * **Everything here is hardcoded.** There is no AI in this repo and no backend:
 * the Define "conversation" is a fixed exchange replayed character-by-character
 * and the brief fills as those fixed replies land. Nothing is generated. See
 * CLAUDE.md §"Scope: hardcoded frontend only".
 *
 ...
 * `lib/content/roadmap-experiment.ts` and `/trial3` has with `trial4`'s
 * sibling. If the concept ships, its data moves behind the queries seam and its
 * recipes move out of `styles/trial4.css`.
 *
 * It duplicates `lib/content/trial3.ts` rather than importing from it, and that
 * is on purpose: two throwaway concept routes that share a module cannot be
 * deleted independently, which is the only thing a concept route is for.
 *
 * The idea itself is the house fixture — the SMS rebooking tool for dental
 * clinics from `lib/fixtures/conversation.ts` — so this can be read against
 * both the shipping Define page and `/trial3` without a second story to hold in
 * mind.
 */

export type Stage = 'define' | 'validate' | 'roadmap';

/** Stage order, used for the rail and for "has the run passed this?" tests. */
export const STAGE_ORDER: Stage[] = ['define', 'validate', 'roadmap'];
...
/** Rail groups, in order. Rendering iterates this, not the sessions. */
export const SESSION_GROUPS = ['Today', 'This week', 'Earlier'];
...
export const OPENING_TURNS: Turn[] = [
  ...
  {
    role: 'assistant',
    text: "Good — that's three fields down the right-hand side already. Now the part everyone skips: when a slot goes empty today, what actually happens to it?",
  },
];
...
/* ---------------------------------------------------------------------------
   The two stages that have no page yet. Both previews are drawn in code — rule
   8 — so what follows is the DATA behind a fragment rather than copy for a
   marketing page.

   The structure is real and the prose is deliberately absent, rendered as
   hairline bars. That is what makes it read as a preview of a real screen
   rather than as a screen that failed to load, and it keeps the product's
   oldest rule intact: nothing is invented to fill a field.
   ------------------------------------------------------------------------ */
...
export type StagePreview = {
  ...
  blurb: string;
  /** Mono. What has to happen before it is real. */
  gate: string;
  /** Mono header of the code-drawn fragment below the stats. */
  fragment: string;
...
export const VALIDATE_PREVIEW: StagePreview = {
  ...
  gate: 'Runs once the brief is approved',
...
export const ROADMAP_PREVIEW: StagePreview = {
  ...
  gate: 'Builds once research completes',
  fragment: 'ROADMAP / PHASES',
  stats: [
...
/**
 * Findings skeleton for the Validate preview. `stance` is the real vocabulary
 * from the shipping report; `bars` are the widths (in %) of the hairline rules
 * standing in for the claim text, so a row reads as a two-line claim without
 * inventing one.
 */
export const VALIDATE_ROWS = [
  { id: 'EV_01', stance: 'Supports', bars: [96, 54] },
  { id: 'EV_02', stance: 'Supports', bars: [88, 71] },
  { id: 'EV_03', stance: 'Contests', bars: [92, 38] },
  { id: 'EV_04', stance: 'Unclear', bars: [78, 62] },
];

/** Phase rows for the roadmap preview. `weeks` positions the bar on the axis. */
...
/** The axis the phase bars are positioned against. */
export const ROADMAP_WEEKS = 18;
